...
string huruf(string str) {
    for (char &c : str)  //cek atu" 
    c = tolower(c); //ubah kecil
    return str; 
}
...
//simpan transaksi sementara AARRAY
...
void stokgudang() {
    ...
    for (int i = 0; i < jmlbar; i++) {
        cout << "| " << setw(2) << right << i + 1 << " "
             ...
             << "| " << setw(8) << stokbarang[i][0] << " " //gudang 1
...
void simpanstokfile(const char* namafile) { //FILE
    FILE* file = fopen(namafile, "w");  //buka buat tulis
    ...
    for (int i = 0; i < jmlbar; i++) {
        fprintf(file, "%d,%d,%d,%s\n", //stok 3 gudang
            stokbarang[i][0], stokbarang[i][1], stokbarang[i][2], tanggalStok[i].c_str()); //ubah const char
    }
    fclose(file);
}
...
void tampildatatransaksi() { //POINTER
    transaksi* ptr = daftartransaksi; //buat pointer ke transaksi
    cout << "====================================================================================\n";
    ...
    for (int i = 0; i < jumlahtransaksi; i++) {
        cout << "| " << setw(2) << i + 1 << " "
             ...
             << "| ";  
        if ((int)(ptr + i)->totalharga == (ptr + i)->totalharga) //total bulat
...
void tampildatatransaksisesi() { //POINTER
    ...
    cout << "| No |   Nama Pembeli   |      Tanggal       |   Nama Barang   | Jumlah |   Total  |\n";
    cout << "====================================================================================\n";
    for (int i = 0; i < jmltransasisesi; i++) { //ben nambah
        cout << "| " << setw(2) << i + 1 << " "
             << "| " << setw(16) << left << (ptr + i)->namapembeli << " "
             << "| " << setw(18) << left << (ptr + i)->tanggalbeli << " "
             << "| " << setw(15) << left << (ptr + i)->namabarang << " "
...
void tambahstok() {
    ...
    if (idxbarang < 1 || idxbarang > jmlbar) { // 1=leptop
    ...
    stokbarang[idxbarang - 1][idxgudang - 1] += jumlahTambah; //all oke = ditambahkan 
    ...
    while (true) { 
        ...
        if (opsi == 'a') {
            ...
            do {
                ...
                cin.ignore();
                getline(cin, keyword);
                bool valid = !keyword.empty(); //ga kosong
                for (char c : keyword) {
                    if (!isalpha(c) && c != ' ') { //hrf gede kecil oke
                        valid = false;
                        break;
                    }
                }
            ...
            for (int i = 0; i < jmlbar; i++) {
                if (huruf(dafbar[i].nama).find(huruf(keyword)) != string::npos) { //cek ada ndak?
                    cout << "| " << setw(2) << right << i + 1 << " "
                         << "| " << setw(12) << left << dafbar[i].nama << " "
                         << "| Rp" << setw(8) << right << dafbar[i].harga << " "
        ...
        } else if (opsi == 'c') { //balek menu utama
            break; 
        } else {
...
void simpantransaksifile(const char* namafile) { //FILE
    FILE* file = fopen(namafile, "w"); //tulisss
...
void ambiltransaksifile(const char* namafile) { //FILE oke
...
void edittransaksi() { //POINTER
    ...
    transaksi* t = & daftartransaksi[no - 1]; //pointer ke transaksi pilih user
...
void hapustransaksi() {
    ...
    if (no < 1 || no > jumlahtransaksi) { //cek no ada plihan
        cout << "Nomor transaksi tidak valid!\n";
        return;
    }
...
void bacafiletransaksi(const char* namafile) { //FILE
    ...
    while (fscanf(file, "%18[^,],%18[^,],%18[^,],%d,%f,%d\n",
        ...
        printf("| %2d | %-16s | %-14s | %-15s | %6d | %8.2f |\n", //ukuran tabel d=kanan, -s=kiri, f=desimal
